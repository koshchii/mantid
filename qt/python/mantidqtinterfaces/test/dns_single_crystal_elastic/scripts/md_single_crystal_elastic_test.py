...
class MDPowderElasticTest(unittest.TestCase):
    ...
    def test_load_all(mock_load_binned):
        ...
        calls = [call('knso_x_nsf', [0, 1, 2], {'a': 1}, 'C:/data',
                      range(554574, 554634, 6), False),
                 call('knso_x_sf', [0, 1, 2], {'a': 1}, 'C:/data',
                      range(554573, 554633, 6), False),
                 call('knso_y_nsf', [0, 1, 2], {'a': 1}, 'C:/data',
                      range(554576, 554636, 6), False),
                 call('knso_y_sf', [0, 1, 2], {'a': 1}, 'C:/data',
                      range(554575, 554635, 6), False),
                 call('knso_z_nsf', [0, 1, 2], {'a': 1}, 'C:/data',
                      range(554578, 554638, 6), False),
                 call('knso_z_sf', [0, 1, 2], {'a': 1}, 'C:/data',
    ...
    def test_load_binned(self, mock_load, mock_binmd, mock_mtd):
        ...
        mock_load.assert_called_once_with(
            ...
            Normalization='monitor', a=2, b=3, c=4, alpha=78, beta=86,
            gamma=85,
            OmegaOffset=0, HKL1='1,2,3', HKL2='2,3,4', LoadAs='raw',
            SaveHuberTo='huber_x_nsf')

        calls = [
            call(InputWorkspace='knso_x_nsf', OutputWorkspace='knso_x_nsf',
                 AxisAligned=True,
                 AlignedDim0='Theta,0.0,0.5,2', AlignedDim1='Omega,3,4,5'),
        ...
        load_binned('knso_x_nsf', binning, params, 'C:/data',
                    range(554574, 554578, 2), True)
        mock_load.assert_called_once_with(
            FileNames='C:/data_554574.d_dat, C:/data_554576.d_dat',
            OutputWorkspace='knso_x_nsf',
            NormalizationWorkspace='knso_x_nsf_norm',
