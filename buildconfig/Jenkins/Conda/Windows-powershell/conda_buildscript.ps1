...
#
#  Script usage:
#  conda-buildscript -workspace <path-to-workspace> -cmake-preset <cmake-preset-name> [options]
#
#  Example command to run a PR build on ubuntu:
#  conda-buildscript -workspace /jenkins/workspace_dir/ -cmake-preset linux-ci --enable-systemtests --enable-coverage --enable-docs
#
#  Expected parameters:
#    -workspace: path to the workspace/source code that this should run inside
#    -cmake-preset: the CMake preset that should be ran to generate the cmake files for this CI job
#
#  Possible flags:
...
param (
    [string] $workspace="N/A",
    [string] $cmake_preset="N/A",
...
if (($workspace -eq "N/A") -or ($cmake_preset -eq "N/A")){
    Write-Output "Please pass a -workspace and -cmake_preset parameter to the script followed by their required information"
}
...
. git clean -d -x --force --exclude=build --exclude=mambaforge --exclude=".Xauthority-*" --exclude=".vscode"
